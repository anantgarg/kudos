...
function isLoggedIn() {
	if (!empty($_SESSION['user']['loggedin']) && !empty($_SESSION['user']['email']) && !empty($_SESSION['user']['type'])) {
		return 1;
	}
...
function authenticate($force = 0) {
	...
	if ($loggedin == 0 && !($controller == 'users' && ($action == 'login' || $action == 'validate' || $action == 'invite' || $action == 'register')) && !($controller == 'cron')&& !($controller == 'oops')) {
